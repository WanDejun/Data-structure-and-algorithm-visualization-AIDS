...
- ps.截至2023.11.23 15:16     已有约4000行代码 （数据结构部分基本完成）  
...
### 树demo (tree_demo.h) ###
- 主要用来测试树的伸展的函数表达式，无数据结构功能

### 树（tree.h） ###
- 在demo的基础上构建
- 包含前序，中序，后序的可视化
- 通过标识圈（marked_cycle）提示当前遍历到的元素

## 一些总结与反思 #
- 低耦合高内聚看着简单，在没有写过数千行，几十个文件时，很难真正在开发中将其做好
- 吐槽：没有面向对象，没有继承关系太痛苦了 :(
- 文件内部函数，全局变量一定要用static标识，通用的typedef和一些定义单独开一个头文件，避免累赘（前几个模块更本没想起来static，痛苦）
- 逻辑性较强的代码随手写注释是个好习惯，其余代码在阶段性完成后也要及时补充注释
